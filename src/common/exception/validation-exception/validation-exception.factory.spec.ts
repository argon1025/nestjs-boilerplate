...
describe('ValidationExceptionFactory', () => {
  it('ValidationPipe 에러를 여러개 받았을 경우', () => {
    ...
    expect(() => GenerateValidationException(errorInfo)).toThrow(
      new BadRequestException({
        ...
        message: 'title must be shorter than or equal to 3 characters',
      }),
    );
  });
  it('ValidationPipe가 빈 배열로 전달되었을 경우', () => {
    ...
    // when&then
    // 기본 에러 메시지를 반환한다.
    expect(() => GenerateValidationException(errorInfo)).toThrow(new BadRequestException(ERROR_CODE.INVALID_PARAMETER));
  });
});
