...
import { ERROR_CODE, ErrorResponseInfo } from '../error-code.constant';
...
describe('AllExceptionFilter', () => {
  ...
  it('사전 정의되지 않은 ERROR 객체를 전달 받았을 경우', () => {
    ...
    expect(mockJsonFunction).toHaveBeenCalledWith(ERROR_CODE.INTERNAL_SERVER_ERROR);
    expect(mockStatusFunction).toHaveBeenCalledWith(500);
  });

  it('사전 정의된 HttpException ERROR 객체를 전달 받았을 경우', () => {
    // given
    const errorInfo: ErrorResponseInfo<'INVALID_PARAMETER'> = {
